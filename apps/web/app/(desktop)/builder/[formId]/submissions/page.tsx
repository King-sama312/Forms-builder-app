...
export default function SubmissionsPage() {
  ...
  const labels = (fields ?? []).reduce((acc: Record<string, string>, f: any) => {
    acc[f.id] = f.label;
    return acc;
  ...
  return (
    <Win98Window
      title={form ? `Submissions - ${form.title}` : 'Submissions'}
      defaultPosition={{ x: 60, y: 40, width: 720, height: 480 }}
              ...
              <tbody>
                {submissions.map((sub: any, i: number) => (
                  <tr key={sub.id} className={i % 2 === 0 ? 'bg-white' : 'bg-[#e0e0e0]'}>
                    ...
                    {(fields ?? []).map((f: any) => {
                      const val = (sub.values ?? []).find(
                        (v: any) => v.formFieldId === f.id
                      );
                      return (
