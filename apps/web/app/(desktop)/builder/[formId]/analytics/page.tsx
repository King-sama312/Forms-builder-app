...
function AnalyticsWindowContent({ formId }: { formId: string }) {
        ...
        <div className="flex-1 flex flex-col gap-2 min-h-0">
          <div className="shrink-0">
            <SummaryStatsRow
              stats={[
                ...
                { label: 'Submissions Today', value: analytics?.submissionsToday ?? 0, icon: '📅' },
                { label: 'Fields', value: form?.fields?.length ?? 0, icon: '📝' },
              ]}
            />
          </div>
          ...
          <div className="flex-1 grid grid-cols-2 gap-2 min-h-0">
            ...
            <ChartPanel title="Field Response Distribution">
              <FieldDistributionsPanel distributions={analytics?.fieldDistributions ?? []} />
            </ChartPanel>
          </div>
        </div>
      )}
